#!/usr/bin/env bash
# =============================================================================
#  setup-github-ubuntu.sh — Automatic GitHub SSH setup for Ubuntu
#  Run with: bash <(curl -fsSL https://raw.githubusercontent.com/YOUR_REPO/main/setup-github-ubuntu.sh)
# =============================================================================
...
echo -e "${BOLD}"
echo "  ╔══════════════════════════════════════════╗"
echo "  ║   GitHub SSH Auto-Setup  •  Ubuntu       ║"
echo "  ╚══════════════════════════════════════════╝"
echo -e "${NC}"

# ── 0. Verify Linux / Ubuntu ──────────────────────────────────────────────────
...
if sudo -n true 2>/dev/null; then
    ...
    warn "No sudo access — will install binaries to ~/.local/bin instead."
fi
...
if command -v git &>/dev/null; then
    ...
    warn "Git not found and no sudo access — skipping git install."
    info "Ask IT to install git, or run: sudo apt-get install git"
fi
...
install_gh_apt() {
    info "Installing gh via official apt repository..."
...
https://cli.github.com/packages stable main" \
    ...
    ok "gh installed via apt: $(gh --version | head -1)"
}

install_gh_binary() {
    info "Downloading gh binary from GitHub Releases (no sudo)..."
    ...
    info "Downloading: $GH_TAR"
    curl -fsSL "$GH_URL" -o "${GH_TMP}/${GH_TAR}"
    tar -xzf "${GH_TMP}/${GH_TAR}" -C "${GH_TMP}"

    GH_BIN=$(find "${GH_TMP}" -name "gh" -type f | head -1)
    [[ -z "$GH_BIN" ]] && err "Could not find gh binary in archive."
...
info "gh binary    : $(command -v gh)"
echo ""
if $HAS_SUDO; then
    echo -e "${YELLOW}Note:${NC} sudo was used to install git and gh system-wide."
else
    echo -e "${YELLOW}Note:${NC} No sudo was used — everything is stored in your home directory: ~/."
fi
echo -e "      Run this script on the next machine to set that up too."
echo ""
